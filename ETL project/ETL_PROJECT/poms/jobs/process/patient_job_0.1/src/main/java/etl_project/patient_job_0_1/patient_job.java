...
package etl_project.patient_job_0_1;
 ...
 * @status
 */
public class patient_job implements TalendJob {
	...
	private final String jobName = "patient_job";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(patient_job.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row_Implicit_Context_RegexStruct
		...
		final static byte[] commonByteArrayLock_ETL_PROJECT_patient_job = new byte[0];
		static byte[] commonByteArray_ETL_PROJECT_patient_job = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_ETL_PROJECT_patient_job.length) {
					if (length < 1024 && commonByteArray_ETL_PROJECT_patient_job.length == 0) {
						commonByteArray_ETL_PROJECT_patient_job = new byte[1024];
					} else {
						commonByteArray_ETL_PROJECT_patient_job = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_ETL_PROJECT_patient_job, 0, length);
				strReturn = new String(commonByteArray_ETL_PROJECT_patient_job, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_ETL_PROJECT_patient_job) {
	...
	public static class d_patient_outputStruct implements routines.system.IPersistableRow<d_patient_outputStruct> {
		...
		public int hashCode() {
			if (this.hashCodeDirty) {
				...
				result = prime * result + (int) this.Id_pat;
		...
		public String toString() {
			...
			sb.append(super.toString());
			sb.append("[");
			sb.append("Id_pat=" + String.valueOf(Id_pat));
		...
		public int compareTo(d_patient_outputStruct other) {
			...
			returnValue = checkNullsAndCompare(this.Id_pat, other.Id_pat);
			if (returnValue != 0) {
				return returnValue;
			}
...
// ###############################
				...
				String url_tDBInput_1 = "jdbc:postgresql://" + context.postgres_DB_input_Server + ":" + "5432" + "/"
				...
				String dbquery_tDBInput_1 = "SELECT * FROM \"Patient\"\nORDER BY \"Id_patient\" ASC ";
...
// # Output table : 'd_patient_output'
							d_patient_output_tmp.Id_pat = row1.Id_patient;
...
///////////////////////		
							...
							strBuffer_tLogRow_1.append(String.valueOf(d_patient_output.Id_pat));

							strBuffer_tLogRow_1.append("|");

							if (d_patient_output.IsFemale != null) { //

								strBuffer_tLogRow_1.append(String.valueOf(d_patient_output.IsFemale));

							} //

							strBuffer_tLogRow_1.append("|");

							if (d_patient_output.Email != null) { //

								strBuffer_tLogRow_1.append(String.valueOf(d_patient_output.Email));

							} //

							strBuffer_tLogRow_1.append("|");

							if (d_patient_output.Age != null) { //

								strBuffer_tLogRow_1.append(String.valueOf(d_patient_output.Age));

							} //

							strBuffer_tLogRow_1.append("|");

							if (d_patient_output.Weight != null) { //

								strBuffer_tLogRow_1.append(FormatterUtils.formatUnwithE(d_patient_output.Weight));

							} //

							strBuffer_tLogRow_1.append("|");

							if (d_patient_output.Size != null) { //

								strBuffer_tLogRow_1.append(String.valueOf(d_patient_output.Size));
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final patient_job patient_jobClass = new patient_job();

		int exitCode = patient_jobClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = patient_job.class.getClassLoader()
					.getResourceAsStream("etl_project/patient_job_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = patient_job.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : patient_job");
		}
...
/************************************************************************************************
 * 82945 characters generated by Talend Open Studio for Big Data on the 24
 * octobre 2025 10:41:27 CEST
 ************************************************************************************************/